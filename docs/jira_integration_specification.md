...
## Инкрементный режим: Jira Snapshot + Diff

### Назначение
Для динамического анализа команды и сравнения нового состояния с предыдущим запуском система фиксирует:
- **Snapshot**: полный нормализованный срез задач Jira на момент запуска
- **Diff**: машинно-обрабатываемая дельта изменений между текущим и предыдущим snapshot

### Где хранятся данные
- Snapshot (источник данных):
  - `data/jira/snapshots/{run_id}.json`

- Diff (артефакт run):
  - `reports/runs/{run_id}/jira-diff/jira-diff.json`

- Diff (артефакт latest):
  - `reports/latest/jira-diff/jira-diff.json`

### Структура diff (высокий уровень)
- `added`: задачи, появившиеся в текущем snapshot
- `removed`: задачи, отсутствующие в текущем snapshot, но бывшие в предыдущем
- `changed`: задачи с изменениями по минимальному fingerprint полей (status/assignee/priority/summary/updated)
- `stats`: агрегированная статистика изменения

### Ответственный компонент
- Формирование snapshot/diff выполняет `ImprovedTaskAnalyzerAgent` как часть run-пайплайна анализа задач.
